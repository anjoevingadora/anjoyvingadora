...
//apikey
const Vhtear = 'YOUR_APIKEY' //beli di https://vhtear.com
const ZeksApi = 'apivinz' //gratis dari https://api.zeks.xyz
const DapApi = 'dappabotwa' //gratis dari https://dapuhy-apikey.herokuapp.com
const OnlyDev = 'YOUR_APIKEY' //beli di https://onlydevcity.herokuapp.com
const TobzKey = 'BotWeA' //gratis dari https://tobz-api.herokuapp.com
const LolHuman = 'YOUR_APIKEY' //beli di http://api.lolhuman.xyz
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `𝗦𝗮𝗶𝘂\n @${num.split('@')[0]} 𝗣𝗼𝗿𝗾𝘂𝗲 𝗣𝗲𝗴𝗼𝘂 𝗖𝗢𝗩𝗜𝗗😣💉`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '[❗] Este é um recurso especial para o BOT❌',
					ownerB: '[❗] Este é um recurso especial para o BOT❌',
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("arrombado")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/arrombado.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
	
	if (messagesC.includes("macumba")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/macumba.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
	
	if (messagesC.includes("patoo")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/patoo.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
 ...
 if (messagesC.includes("chuvendo")){
	...
	if (messagesC.includes("gatinha")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/gatinha.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
    ...
    break
		...
		if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					jds = []
					const jdiiii = groupMembers
					const kossss = groupMembers
                    const quaa = groupMembers
					const lindyy = groupMembers
					const cincoo = groupMembers
					const akuuuu = jdiiii[Math.floor(Math.random() * jdiiii.length)]
					const diaaaa = kossss[Math.floor(Math.random() * kossss.length)]
					const quatroo = quaa[Math.floor(Math.random() * quaa.length)]
					const trooot = lindyy[Math.floor(Math.random() * lindyy.length)]	
					const cinccor = cincoo[Math.floor(Math.random() * cincoo.length)]										
					teks = `*TOP 5 +GADOS DO GRUPO*\n\n🐂├> @${akuuuu.jid.split('@')[0]}\n🐂├> @${diaaaa.jid.split('@')[0]}\n🐂├> @${quatroo.jid.split('@')[0]}\n🐂├> @${trooot.jid.split('@')[0]}\n🐂└> @${cinccor.jid.split('@')[0]}\n\n *Daddy-Bot*`
					jds.push(akuuuu.jid)
					jds.push(diaaaa.jid)
					jds.push(quatroo.jid)
					jds.push(trooot.jid)		
					jds.push(cinccor.jid)										
